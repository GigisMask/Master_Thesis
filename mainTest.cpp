...
int main()
{
    ...
    for (i = 0; i < Nx; i++)
    {
        for (j = 0; j < Ny; j++)
        {
            ...
            Psi_p[i*Ny + j][REAL] *= sign;
            Psi_p[i*Ny + j][IMAG] *= sign;
        }
    }
    ...
    for (int i = 0; i < Nx; i++) // Normalize
        for (int j = 0; j < Ny; j++)
        {
            Psi[i*Ny + j][REAL] = 1. / (Nx * Ny) * Psi[i*Ny + j][REAL];
            Psi[i*Ny + j][IMAG] = 1. / (Nx * Ny) * Psi[i*Ny + j][IMAG];
        }
    ...
    for (int j = 0; j < Ny; j++)
    {
        ...
        {
            //pot_out << V[i * Ny + j] << "\t";
            pot_out << Psi_p[i*Ny + j][REAL]*Psi_p[i*Ny + j][REAL] + Psi_p[i*Ny + j][IMAG]*Psi_p[i*Ny + j][IMAG] << "\t";
            //pot_out << Psi[i*Ny + j][REAL]*Psi[i*Ny + j][REAL] + Psi[i*Ny + j][IMAG]*Psi[i*Ny + j[IMAG] << "\t";
        }
        pot_out << std::endl;
    }
    free(V);
